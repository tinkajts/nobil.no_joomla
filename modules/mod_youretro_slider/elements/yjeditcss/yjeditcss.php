...
defined('_JEXEC') or die('Restricted access');
		...
		$path		= JPATH_ROOT.DS."modules".DS.$yj_mod_name.DS."css";
		$filter		= '.css';// css files only
		$exclude	= '';
		$stripExt	= '';
		$files		= JFolder::files($path, $filter);

		$options = array ();
		$options[] = JHTML::_('select.option', '', '- '.JText::_('Select CSS File').' -');
...
////////////////////////////////////////////////////////////////////
		if(isset($_POST['yjme_selected'])){
			...
			global $yjme_name_only;
			$yjme_name_only 			= "stylesheet.css";
		}
		global $yjme_file_dir;
		$yjme_file_dir 			= JPATH_ROOT."modules".DS.$yj_mod_name.DS."css".DS;
		global $yjme_file;
	...
	function getedit_file(){
		...
		$yjme_file_dir 			= JPATH_ROOT.DS."modules".DS.$yj_mod_name.DS."css".DS;
		$yjme_file 				= $yjme_file_dir.$yjme_name_only;
...
///////////////////////////////////////////////////////////////////
		...
		if(!isset($_POST['yjme_file_content']) && !isset($_POST['yjme_selected'])){
			$yjme_file_yjc 			= JText::_('SELECT_CSS_FILE');
			$yjme_file_yjn 			= '';
		}
