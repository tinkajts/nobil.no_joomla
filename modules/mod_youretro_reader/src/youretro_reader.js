	...
	start: function(){
		...
		*/
		this.slides.each(function(slide, i){
			...
			this.slides[i]['fx'] = new Fx.Morph(slide, {link:'cancel', duration:this.options.slides.slideSpeed, transition:this.options.slides.slideTranz,wait:false});
...
window.addEvent('load', function() {
  ...
  var yrreadon = $$('.yrreadon');
  var yrreadonspan = $$('.yrreadon span');
  var yrrtrigger=$$('.youretro_readon');
  
  
  
  yrreadonspan.each(function(element) {
	  var fx = new Fx.Morph(element, {duration:700, wait:false});
	  
	  
	  yrrtrigger.addEvent('mouseenter', function(){
		  fx.start({
			  'opacity':[0,1],
			  'display':'block',
			  'visibility':'visible'
	  ...
	  yrrtrigger.addEvent('mouseleave', function(){
		  fx.start({
			  'opacity':[1,0]
		  });
	  });
